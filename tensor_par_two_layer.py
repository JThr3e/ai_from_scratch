...
if __name__ == "__main__":
    ...
    grad_dtype = np.float64
    a3_shape=(1, output_size)
    a2_shape=(1, hidden_size2)
    a1_shape=(1, hidden_size)

    a3_shm =shared_memory.SharedMemory(create=True, size=np.prod(a3_shape) * np.dtype(grad_dtype).itemsize)
    a2_shm =shared_memory.SharedMemory(create=True, size=np.prod(a2_shape) * np.dtype(grad_dtype).itemsize)
    a1_shm =shared_memory.SharedMemory(create=True, size=np.prod(a1_shape) * np.dtype(grad_dtype).itemsize)

    act_mem_names = [a3_shm.name, a2_shm.name, a1_shm.name]
    ...
    for i in range(0, num_processes):
        p = mp.Process(target=dp_worker, args=(i, lock, act_mem_names, sizes, x_train, t_train_proc, 100, 0.01))
        p.start()
